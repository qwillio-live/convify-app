...
export const ListItem = ({ listItemStyles, item }) => {
  return (
    <div
...
export const List = ({ listStyles, listItemStyles, listItems, ...props }) => {
  const {
    actions: { setProp },
      ...
      {listItems.map((item) => (
        ...
        >
          <div className="flex items-center gap-x-4">
            ...
            <div className="flex text-lg flex-col my-1 flex-1">
              <ContentEditable
                html={item.text}
                disabled={!editable}
                onChange={(e) =>
                  setProp((props) => {
                    if (props && props.item) {
                      props.item.text = e.target.value.replace(
                        /<\/?[^>]+(>|$)/g,
                        ""
                      )
                    }
              ...
              <ContentEditable
                ...
                onChange={(e) =>
                  setProp((props) => {
                    if (props && props.item) {
                      props.item.subText = e.target.value.replace(
                        /<\/?[^>]+(>|$)/g,
                        ""
                      )
                    }
