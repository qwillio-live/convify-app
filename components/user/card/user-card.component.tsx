...
const CardContentOuter = styled.div<CardOuterStyles>`
  ...
  /* border: ${(props) => (props.selected ? "1px dotted" : "0")} solid ${(
    props
  ) => props.borderColor}; */
  border-color: ${(props) =>
    props.selected || props.parentHovered ? "#60A5FA" : props.backgroundColor};
  border-style: ${(props) =>
    ...
    >
      ...
      >
        {children ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 text-current opacity-50">
            <Plus size={32} /> {t("Add blocks here")}
          </div>
        )}
        {/* {children} */}
      </CardContentGen>
...
export const Card = ({ children, backgroundColor, ...props }) => {
  ...
  return (
    <div
      ref={(ref: any) => connect(drag(ref))}
      // backgroundColor={props.backgroundColor}
      className="card-container relative flex min-w-full shrink-0 basis-full flex-col items-center justify-center border-0"
      {...props}
