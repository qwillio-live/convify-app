...
} from "../placeholderScreensSlice"
import { PayloadAction } from "@reduxjs/toolkit"
import { RootState } from "../../store"

function* applyThemeAndCycleScreens(action: PayloadAction<any>) {
  const originalSelectedScreen: number = yield select(
    (state: RootState) => state?.screen?.selectedScreen
  )
  console.log("SAGA ORIGINAL WAS: ", originalSelectedScreen)
  const screens: any[] = yield select(
    (state: RootState) => state?.screen?.screens
  )
...
export function* applyThemeBackgroundAndCycleScreens(
  action: PayloadAction<any>
) {
  const screens: any[] = yield select((state) => state?.screen?.screens)

  // Set partial theme styles
  yield put(setBackgroundColors(action.payload))
  ...
  for (let i = 0; i < screens.length; i++) {
    ...
    // modify background
    const modifiedScreen: string = yield call(
      modifyBackground,
      action.payload,
      screenJson
    )
