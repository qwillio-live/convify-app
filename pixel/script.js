document.addEventListener('DOMContentLoaded', function () {
  ...
  const API_URL = 'https://conv-kamran.picreel.bid';
  ...
  function sendResponseEvent(stepId, content, responseId = null, state) {
    ...
    const url = responseId ? `${API_URL}/api/flows/${flowId}/responses/${responseId}` : `${API_URL}/api/flows/${flowId}/responses`;
    const data = {
      ...storage,
      ...content,
    };
  ...
  function wasVisitedInLast24Hours(stepId) {
    const visitedTime = getLocalStorageItem(`${VISITED_STORAGE_PREFIX}${stepId}`);
    if (visitedTime) {
  ...
  function handleStepVisit(stepId) {
    if (!wasVisitedInLast24Hours(stepId)) {
      ...
      localStorage.setItem(`${VISITED_STORAGE_PREFIX}${stepId}`, new Date().toISOString()); // 1440 minutes = 24 hours
    }
  }
  ...
  function buildContentData() {
    ...
    if (activeSection) {
      ...
      elementsMulti.forEach((element) => {
        ...
        let parentText;
        try {
          parentText = element.childNodes[0].textContent.replace(/\n/g, '').replace(/  /g, '');
        } catch (e) {}
        const label = element.getAttribute('data-label') || parentText || element.name || element.id;
        let values = [];
        ...
        elements.forEach((element) => {
          ...
          let parentText;
          try {
            parentText = element.parentElement.textContent.replace(/\n/g, '').replace(/  /g, '');
          } catch (e) {}
          const label = element.getAttribute('data-label') || (labelElement ? labelElement.innerText : parentText || element.name || element.id);
          let value;
      ...
      elements.forEach((element) => {
        ...
        let parentText;
        try {
          parentText = element.parentElement.parentElement.textContent.replace(/\n/g, '').replace(/  /g, '');
        } catch (e) {}
        const label = element.getAttribute('data-label') || (labelElement ? labelElement.innerText : parentText || element.name || element.id);
        let value;
  ...
  document.addEventListener('click', (event) => {
    ...
    const responseInsideTag = singleTag ? singleTag.querySelector(`.${RESPONSE_BUTTON_CLASS}`) : false;

    const button = target.tagName === 'BUTTON' ? target : target.closest('button');

    if ((button && button.classList.contains(RESPONSE_BUTTON_CLASS)) || responseInsideTag) {
      const stepId = getStepIdFromHash();
      if (stepId) {
